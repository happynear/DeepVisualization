function [style_generate_prototxt, style_pattern, content_pattern, obj] = MakeStylePrototxt( original_file, weight_file, style_layer, style_weights, content_layer, style_image, content_image, num_cluster )
%MAKESTYLEPROTOTXT 此处显示有关此函数的摘要
%   此处显示详细说明
    
    addpath('../PrototxtGen');
    original_net_model = fileread(original_file);
    original_net_model = strrep(original_net_model,'AVE','MAX');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%color prior
if num_cluster > 0
    obj = getColorPrior(style_image,num_cluster);
    fid = fopen('gaussian_net.prototxt','w');
    gaussian_net_template_file = 'gaussian_net_template.prototxt';
    gaussian_net_template = fileread(gaussian_net_template_file);
    
    gmm_layer = strrep(gaussian_net_template,'{height}',num2str(height));
    gmm_layer = strrep(gmm_layer,'{width}',num2str(width));
    gmm_layer = strrep(gmm_layer,'{output_num}',num2str(obj.NComponents * 3));
    fprintf(fid,'%s\r\n',gmm_layer);
    fclose(fid);
else
    obj = [];
